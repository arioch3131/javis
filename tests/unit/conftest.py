...
def mock_loggable_mixin_global(monkeypatch):
    ...
    """
    monkeypatch.setattr('ai_content_classifier.core.logger.LoggableMixin', MockLoggableMixin)

@pytest.fixture(scope='session')
def setup_database():
    """Set up an in-memory SQLite database for testing."""
    engine = create_engine('sqlite:///:memory:', echo=False)  # Set echo=True for SQL debugging
    
    print(f"Tables known to Base.metadata before create_all: {list(Base.metadata.tables.keys())}")
    Base.metadata.create_all(engine)
    print(f"Tables known to Base.metadata after create_all: {list(Base.metadata.tables.keys())}")
    
    # Verify expected tables are created
    expected_tables = {'content_items', 'tags', 'collections', 'app_settings', 'content_tags', 'collection_contents'}
