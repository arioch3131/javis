...
class TestThumbnailService(unittest.TestCase):
    ...
    @patch('ai_content_classifier.services.shared.dependency_manager.get_dependency_manager')
    ...
    def test_generator_selection(self):
        ...
        with patch('ai_content_classifier.services.thumbnail.thumbnail_service.QT_AVAILABLE', True):
    ...
    def test_caching_behavior(self):
        ...
        config = ThumbnailConfig(enable_caching=True, max_workers=1, propagate_logs=False)
        
        with patch('ai_content_classifier.services.thumbnail.thumbnail_service.SmartPoolHandle') as mock_handle:
            ...
            mock_cache_instance.acquire.return_value = (2, 'key', "cached_thumbnail_data")
...
class TestBatchProcessing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.temp_dir = tempfile.mkdtemp()
        self.config = ThumbnailConfig(enable_caching=False, max_workers=2, propagate_logs=False)
    ...
    @patch('ai_content_classifier.services.thumbnail.thumbnail_service.ThumbnailService.create_thumbnail')
    ...
    @patch('ai_content_classifier.services.thumbnail.thumbnail_service.ThumbnailService.create_thumbnail_async')
    def test_create_thumbnails_batch_async(self, mock_create_thumbnail_async):
        ...
        def side_effect(image_path, callback, size):
            res = ThumbnailResult(
                success=True, path=image_path, thumbnail="data", size_str="1 KB", file_size=1024
            )
            callback(res)
...
class TestAsyncFeatures(unittest.TestCase):
    ...
    def test_create_thumbnail_async_standard(self, mock_submit):
        ...
        with patch.object(self.service, 'create_thumbnail') as mock_create:
            mock_create.return_value = ThumbnailResult(success=True, path=str(dummy_path), thumbnail="data", size_str="10 B", file_size=10)
    ...
    def test_create_thumbnail_async_progressive(self, mock_submit):
        ...
        with patch.object(self.service, 'create_thumbnail') as mock_create:
            mock_create.side_effect = [
                ThumbnailResult(success=True, path=str(large_file_path), quality=0.5, thumbnail="low_q", size_str="200 B", file_size=200),
                ThumbnailResult(success=True, path=str(large_file_path), quality=1.0, thumbnail="high_q", size_str="200 B", file_size=200)
            ]
...
class TestVirtualization(unittest.TestCase):
    ...
    @patch('os.makedirs')
    @patch('ai_content_classifier.services.thumbnail.thumbnail_service.ThumbnailService.create_thumbnail')
    def test_virtualization_success_qt(self, mock_create_thumbnail, mock_makedirs, mock_exists):
        ...
        mock_create_thumbnail.return_value = ThumbnailResult(
            success=True, path="", thumbnail=mock_pixmap_instance, size_str="", file_size=0
        )

        self.service.use_qt = True
        with patch('ai_content_classifier.services.thumbnail.thumbnail_service.QPixmap', MockQPixmap):
            result_path = self.service.create_thumbnail_for_virtualization("/path/to/image.jpg")
    ...
    @patch('os.makedirs')
    @patch('ai_content_classifier.services.thumbnail.thumbnail_service.ThumbnailService.create_thumbnail')
    def test_virtualization_success_pil(self, mock_create_thumbnail, mock_makedirs, mock_exists):
