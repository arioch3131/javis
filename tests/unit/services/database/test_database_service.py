...
class TestDatabaseService:
    ...
    def mock_create_engine(self):
        with patch('ai_content_classifier.services.database.database_service.create_engine') as mock_engine:
            yield mock_engine

    @pytest.fixture
    def mock_event_listens_for(self):
        with patch('ai_content_classifier.services.database.database_service.event.listens_for') as mock_listens_for:
            yield mock_listens_for

    @pytest.fixture
    def mock_base_metadata(self):
        with patch('ai_content_classifier.models.base.Base.metadata') as mock_metadata:
            yield mock_metadata

    @pytest.fixture
    def mock_scoped_session(self):
        with patch('ai_content_classifier.services.database.database_service.scoped_session') as mock_scoped:
            yield mock_scoped

    @pytest.fixture
    def mock_sessionmaker(self):
        with patch('ai_content_classifier.services.database.database_service.sessionmaker') as mock_sm:
            yield mock_sm

    def test_creates_distinct_instances(self, mock_create_engine, mock_event_listens_for, mock_base_metadata, mock_scoped_session, mock_sessionmaker):
    ...
    def test_initialize_engine_success(self, mock_create_engine, mock_event_listens_for, mock_base_metadata, mock_scoped_session, mock_sessionmaker):
        ...
        mock_base_metadata.create_all.assert_called_once_with(mock_engine_instance)
        ...
        db_service.logger.info.assert_called_with(f"Database successfully initialized at {TEST_DB_PATH}.")

    def test_initialize_engine_applies_schema_updates(self, mock_create_engine, mock_event_listens_for, mock_base_metadata, mock_scoped_session, mock_sessionmaker):
        mock_engine_instance = mock_create_engine.return_value
        mock_connection = MagicMock()
        mock_begin = MagicMock()
        mock_begin.__enter__.return_value = mock_connection
        mock_begin.__exit__.return_value = None
        mock_engine_instance.begin.return_value = mock_begin
        ...
        mock_connection.exec_driver_sql.assert_any_call("PRAGMA table_info(content_items)")
        mock_connection.exec_driver_sql.assert_any_call(
    ...
    def test_set_sqlite_pragma_success(self, mock_create_engine, mock_event_listens_for):
        ...
        set_sqlite_pragma_func(mock_dbapi_connection, None) # Call the decorated function
    ...
    def test_set_sqlite_pragma_failure(self, mock_create_engine, mock_event_listens_for):
        mock_listens_for_return_value = MagicMock()
        mock_event_listens_for.return_value = mock_listens_for_return_value
        ...
        db_service.logger.warning.assert_called_once_with("Failed to set SQLite pragmas: Pragma error")
        mock_cursor.close.assert_called_once()

    # Test get_session context manager
    def test_get_session_success(self, mock_create_engine, mock_event_listens_for, mock_base_metadata, mock_scoped_session, mock_sessionmaker):
        db_service = DatabaseService(TEST_DB_PATH)
        mock_session_instance = MagicMock(spec=Session)
        mock_scoped_session.return_value.return_value = mock_session_instance # This is the session factory
    ...
    def test_get_session_rollback_on_exception(self, mock_create_engine, mock_event_listens_for, mock_base_metadata, mock_scoped_session, mock_sessionmaker):
        db_service = DatabaseService(TEST_DB_PATH)
        mock_session_instance = MagicMock(spec=Session)
    ...
    def test_close_all_success(self, mock_create_engine, mock_event_listens_for, mock_base_metadata, mock_scoped_session, mock_sessionmaker):
        db_service = DatabaseService(TEST_DB_PATH)
        ...
        db_service.logger.info.assert_called_with("All database connections successfully closed.")
        db_service.logger.error.assert_not_called()

    def test_close_all_error(self, mock_create_engine, mock_event_listens_for, mock_base_metadata, mock_scoped_session, mock_sessionmaker):
        db_service = DatabaseService(TEST_DB_PATH)
        mock_scoped_session.return_value.remove.side_effect = Exception("Close error")
    
        db_service.close_all()
    
        db_service.logger.error.assert_called_once_with("Error during database connection closure: Close error")
