...
from ai_content_classifier.services.database.content_database_service import ContentDatabaseService
from ai_content_classifier.repositories.content_repository import ContentRepositoryFactory
from ai_content_classifier.services.database.operations.enhanced_reader import EnhancedContentReader
...
class TestContentDatabaseService:
    ...
    def mock_content_repository_factory(self):
        mock_factory = MagicMock(spec=ContentRepositoryFactory) # Use spec for better mocking
    ...
    def setup_mocks(self, mock_database_service, mock_query_optimizer, mock_performance_metrics,
                    mock_content_repository_factory, mock_enhanced_content_reader, mock_content_writer):
        with ExitStack() as stack:
            stack.enter_context(patch('ai_content_classifier.services.database.content_database_service.ContentRepositoryFactory', mock_content_repository_factory))
            stack.enter_context(patch('ai_content_classifier.services.database.content_database_service.EnhancedContentReader', mock_enhanced_content_reader))
            stack.enter_context(patch('ai_content_classifier.services.database.content_database_service.ContentWriter', mock_content_writer))
    ...
    def test_initialization(self, mock_database_service, mock_query_optimizer, mock_performance_metrics,
                            mock_content_repository_factory, mock_enhanced_content_reader, mock_content_writer):
        ...
        mock_enhanced_content_reader.assert_called_once_with(mock_database_service, mock_query_optimizer, mock_performance_metrics)
        mock_content_writer.assert_called_once_with(mock_database_service, self.service.repos)
    ...
    def test_create_unit_of_work(self):
        ...
        assert unit_of_work == self.mock_content_writer.return_value.repos.create_unit_of_work.return_value
    ...
    def test_force_database_sync_exception_during_execute(self, mock_text):
        ...
        self.service.force_database_sync()
        self.service.logger.debug.assert_called_with("Database sync failed (this is normal for non-SQLite or specific configurations): Test execute error")
    ...
    def test_force_database_sync_exception_during_session_acquisition(self):
        ...
        self.service.force_database_sync()
        self.service.logger.debug.assert_called_with("Could not force database sync due to session acquisition error: Test session error")

    def test_count_all_items(self):
        mock_session = MagicMock()
        self.service.count_all_items(mock_session)
        self.mock_enhanced_content_reader.return_value.count_all_items.assert_called_once_with(mock_session)
    ...
    def test_get_content_by_path(self):
        ...
        self.mock_enhanced_content_reader.return_value.get_content_by_path.assert_called_once_with(path, None)
