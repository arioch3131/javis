...
class TestConfigService:
    ...
    def test_initialize_default_settings(self, config_service, mock_config_repository):
        mock_config_repository.get_value.side_effect = [None, None, None, None] # Simulate all values as non-existent
        ...
        mock_config_repository.set_value.assert_any_call(ConfigKey.API_URL.value, "http://localhost:11434")

        # Check calls for IMAGE_MODEL (None -> set)
        mock_config_repository.get_value.assert_any_call(ConfigKey.IMAGE_MODEL.value)
        mock_config_repository.set_value.assert_any_call(ConfigKey.IMAGE_MODEL.value, "llava")

        # Check calls for CATEGORIES (None -> set)
        mock_config_repository.get_value.assert_any_call(ConfigKey.CATEGORIES.value)
        mock_config_repository.set_value.assert_any_call(ConfigKey.CATEGORIES.value, "cat1,cat2")

        # Check calls for THUMBNAIL_SIZE (None -> set)
        mock_config_repository.get_value.assert_any_call(ConfigKey.THUMBNAIL_SIZE.value)
        mock_config_repository.set_value.assert_any_call(ConfigKey.THUMBNAIL_SIZE.value, "128")
    ...
    def test_get_from_repository_and_cache(self, config_service, mock_config_repository):
        ...
        mock_config_repository.get_value.assert_called_once_with(ConfigKey.API_URL.value, "http://localhost:11434")
    ...
    def test_get_type_conversion_list(self, config_service, mock_config_repository):
        ...
        assert config_service._cache.get(ConfigKey.CATEGORIES.value) == ["item1", "item2", "item3"]
    ...
    def test_get_invalid_key(self, config_service):
        with pytest.raises(ValueError, match="Invalid configuration key"): # Use a non-existent key
            config_service.get(ConfigKey.DOCUMENT_MODEL) # Assuming DOCUMENT_MODEL is not in mock_config_definitions

    def test_set_value(self, config_service, mock_config_repository):
        config_service.set(ConfigKey.API_URL, "new_url")
        mock_config_repository.set_value.assert_called_once_with(ConfigKey.API_URL.value, "new_url")
    ...
    def test_set_list_value(self, config_service, mock_config_repository):
        ...
        mock_config_repository.set_value.assert_called_once_with(ConfigKey.CATEGORIES.value, "new_cat1,new_cat2")
        assert config_service._cache.get(ConfigKey.CATEGORIES.value) == ["new_cat1", "new_cat2"]
