...
from ai_content_classifier.services.metadata.extractors.hachoir_extractor import HachoirExtractor
...
class TestHachoirExtractor(unittest.TestCase):
    ...
    def test_check_dependency(self):
        ...
        self.extractor._check_dependency = HachoirExtractor._check_dependency.__get__(self.extractor)
    ...
    def test_get_metadata_without_hachoir(self):
        ...
        # Get metadata
        with patch.object(self.extractor, 'get_basic_metadata', return_value={'filename': 'test.pdf'}):
    ...
    def test_extract_hachoir_metadata(self, mock_extract_metadata, mock_create_parser):
        ...
        mock_meta_group1.__iter__.return_value = [mock_meta_item1, mock_meta_item2, mock_meta_item3]
    ...
    def test_extract_docx_metadata(self, mock_document):
        ...
        mock_doc = MagicMock()
        mock_doc.paragraphs = [MagicMock(text="First paragraph"), MagicMock(text="Second paragraph")]
        ...
        self.assertEqual(result['text_statistics']['word_count'], 4)  # "First paragraph Second paragraph"
    ...
    def test_process_metadata_item(self):
        ...
        # Process the item
        self.extractor._process_metadata_item(meta_item, metadata, group_name, extracted_dates)
        ...
        # Process date item
        self.extractor._process_metadata_item(date_item, metadata, "Common", extracted_dates)
        ...
        self.extractor._process_metadata_item(multi_item, metadata, "Common", extracted_dates)
    ...
    def test_get_metadata_with_error_in_hachoir_extraction(self):
        ...
        with patch.object(self.extractor, 'get_basic_metadata', return_value=basic_metadata):
            with patch.object(self.extractor, '_extract_hachoir_metadata', side_effect=Exception("Hachoir error")):
    ...
    def test_get_metadata_with_pdf_extraction_error(self):
        ...
        with patch.object(self.extractor, 'get_basic_metadata', return_value=basic_metadata):
            with patch.object(self.extractor, '_extract_hachoir_metadata', return_value=hachoir_metadata):
                with patch.object(self.extractor, '_extract_pdf_metadata', side_effect=Exception("PDF error")):
                    metadata = self.extractor.get_metadata('test.pdf')
    ...
    def test_get_metadata_with_docx_extraction_error(self):
        ...
        basic_metadata = {'filename': 'test.docx', 'mime_type': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'}
        hachoir_metadata = {'width': 1024}
        
        with patch.object(self.extractor, 'get_basic_metadata', return_value=basic_metadata):
            with patch.object(self.extractor, '_extract_hachoir_metadata', return_value=hachoir_metadata):
                with patch.object(self.extractor, '_extract_docx_metadata', side_effect=Exception("DOCX error")):
    ...
    def test_process_metadata_item_date_processing_error(self):
        ...
        with patch.object(self.extractor.logger, 'debug') as mock_debug:
            self.extractor._process_metadata_item(date_item, metadata, group_name, extracted_dates)
    ...
    def test_check_dependency_import_error(self):
        ...
        extractor._check_dependency = HachoirExtractor._check_dependency.__get__(extractor)
        
        # Test with a module that definitely doesn't exist
        self.assertFalse(extractor._check_dependency('definitely_non_existent_module_12345'))
    ...
    def test_initialization_without_hachoir(self):
        ...
        with patch.object(HachoirExtractor, '_check_dependency') as mock_check:
            ...
            mock_check.side_effect = lambda module: module != 'hachoir.parser' and module != 'hachoir.metadata'
            ...
            with patch('ai_content_classifier.services.metadata.extractors.base_extractor.LoggableMixin.__init_logger__', mock_init_logger):
    ...
    @patch('docx.Document')
    def test_extract_docx_metadata_core_properties_attribute_access(self, mock_document):
    ...
    def test_get_metadata_with_pdf_extraction_when_available(self):
        ...
        with patch.object(self.extractor, 'get_basic_metadata', return_value=basic_metadata):
            with patch.object(self.extractor, '_extract_hachoir_metadata', return_value=hachoir_metadata):
                with patch.object(self.extractor, '_extract_pdf_metadata', return_value=pdf_metadata) as mock_pdf:
                    metadata = self.extractor.get_metadata('test.pdf')
    ...
    def test_get_metadata_with_docx_extraction_when_available(self):
        ...
        with patch.object(self.extractor, 'get_basic_metadata', return_value=basic_metadata):
            with patch.object(self.extractor, '_extract_hachoir_metadata', return_value=hachoir_metadata):
                with patch.object(self.extractor, '_extract_docx_metadata', return_value=docx_metadata) as mock_docx:
                    metadata = self.extractor.get_metadata('test.docx')
