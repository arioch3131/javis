...
from ai_content_classifier.services.metadata.extractors.pillow_extractor import (
    PillowImageExtractor,
)
...
class TestPillowImageExtractor(unittest.TestCase):
    ...
    def test_get_metadata_basic_image_info(self, mock_image_open):
        ...
        # Get metadata
        with patch.object(
            self.extractor, "get_basic_metadata", return_value={"filename": "test.jpg"}
        ):
            metadata = self.extractor.get_metadata("test.jpg")
    ...
    def test_get_metadata_with_exif_data(self, mock_image_open):
        ...
        with patch.object(
            ...
            with (
                patch("PIL.ExifTags.TAGS", exif_tags),
                patch("PIL.ExifTags.GPSTAGS", gps_tags),
            ):
                metadata = self.extractor.get_metadata("test.jpg")
    ...
    def test_extract_date_from_exif(self, mock_image_open):
        ...
        metadata = {"exif": {"DateTimeOriginal": date_str}}
    ...
    def test_process_gps_coordinates(self):
        ...
        self.assertAlmostEqual(
            metadata["gps"]["decimal_longitude"], -118.420833, places=3
        )
    ...
    def test_generate_caption(self):
        ...
        minimal_metadata = {"filename": "test.jpg"}
    ...
    def test_get_metadata_exif_tags_import_error(self, mock_image_open):
        ...
        with (
            patch("PIL.ExifTags.TAGS", new=None),
            patch("PIL.ExifTags.GPSTAGS", new=None),
        ):
            with patch.object(
                self.extractor,
                "get_basic_metadata",
                return_value={"filename": "test.jpg"},
            ):
                metadata = self.extractor.get_metadata("test.jpg")
    ...
    def test_sanitize_exif_value_bytes(self):
        ...
        self.assertEqual(
            self.extractor._sanitize_exif_value(b"some_bytes"), "binary_data"
        )
    ...
    def test_process_gps_coordinates_exception(self):
        ...
        self.assertEqual(
            ...
            "https://maps.google.com/maps?q=bad_lat,bad_lon",
        )

    def test_convert_to_decimal_degrees_non_tuple(self):
        """Test _convert_to_decimal_degrees with non-tuple input."""
        self.assertEqual(
            self.extractor._convert_to_decimal_degrees("not_a_tuple"), "not_a_tuple"
        )

    @patch(
        "PIL.ImageCms.getOpenProfile", side_effect=Exception("Simulated ImageCms error")
    )
    @patch(
        "PIL.ImageCms.getProfileDescription",
        side_effect=Exception("Simulated ImageCms error"),
    )
    def test_extract_icc_profile_exception(
        self, mock_get_profile_description, mock_get_open_profile
    ):
    ...
    @patch.object(
        ...
        side_effect=Exception("Quality estimate error"),
    )
    ...
    def test_process_tiff_specific_import_error(self):
        ...
        with patch("builtins.__import__", side_effect=mock_import):
            ...
            self.assertFalse(
                result
            )  # Should return empty dict as no tiff_info is added
    ...
    def test_get_metadata_unsupported_format(self, mock_image_open):
        ...
        with patch.object(
            self.extractor, "get_basic_metadata", return_value={"filename": "test.xyz"}
        ):
