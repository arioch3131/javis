...
class TestLLMService(unittest.TestCase):
    def setUp(self):
        ...
        self.assertEqual(
            self.mock_config_service.get(ConfigKey.IMAGE_MODEL), "mock_image_model"
        )
        self.assertEqual(
            self.mock_config_service.get(ConfigKey.DOCUMENT_MODEL),
            "mock_document_model",
        )
        self.assertEqual(
            self.mock_config_service.get(ConfigKey.API_URL), "http://localhost:11434"
        )
        ...
        with (
            patch(
                "ai_content_classifier.services.llm.llm_service.LLMApiClient"
            ) as mock_api_client_class,
            patch(
                "ai_content_classifier.services.llm.llm_service.ModelManager"
            ) as mock_model_manager_class,
            patch(
                "ai_content_classifier.services.llm.llm_service.CategoryAnalyzer"
            ) as mock_category_analyzer_class,
        ):
            # Configure mocks
            ...
            self.assertEqual(
                self.service.config_service.get(ConfigKey.IMAGE_MODEL),
                "mock_image_model",
            )
            self.assertEqual(
                self.service.config_service.get(ConfigKey.API_URL),
    ...
    def test_initialization_category_analyzer_failure(self, mock_analyzer):
        ...
        with (
            patch("ai_content_classifier.services.llm.llm_service.LLMApiClient"),
    ...
    def test_update_config_url_changed(self):
        ...
        with (
            ...
            mock_api_client_class.assert_called_once_with(
                new_url, self.service.config_service
            )
    ...
    def test_test_connection_success(self):
        ...
        self.mock_api_client.check_connection.return_value = (
            True,
    ...
    def test_test_connection_with_different_url(self):
        ...
        with patch(
            ...
            mock_client_class.assert_called_once_with(
                test_url, self.mock_config_service
            )
    ...
    def test_classify_image_success(self):
        ...
        self.mock_category_analyzer.extract_category_with_confidence.return_value = (
    ...
    def test_classify_document_success(self):
        ...
        result = self.service.classify_document(
            "/path/to/document.pdf", ["finance", "technology"]
        )
    ...
    def test_classify_image_uses_structured_json_response(self):
        ...
        self.mock_model_manager.ensure_model_available.return_value = True
        self.mock_model_manager.get_model_info.return_value = None
        self.mock_api_client.generate.return_value = (
            '{"category":"animal","confidence":0.82}'
        )
    ...
    def test_extract_category_from_structured_response_invalid_confidence_defaults(
        self,
    ):
        result = self.service._extract_category_from_structured_response(
    ...
    def test_classify_content_cached_result(self):
        ...
        self.assertEqual(
            self.service.config_service.get(ConfigKey.IMAGE_MODEL), "mock_image_model"
        )

        # Pre-populate cache with exactly the same parameters
        cached_result = ClassificationResult(
            category="cached_category", confidence=0.95, cache_hit=False
        )
        ...
        if result.category != "cached_category":
            ...
            actual_cache_key = self.service._make_cache_key(
                path, categories, actual_model
            )
    ...
    def test_classify_content_fallback_extraction(self):
        ...
        self.mock_model_manager.ensure_model_available.return_value = True
        self.mock_api_client.generate.return_value = (
            "This is clearly about animals and cats"
        )

        result = self.service.classify_image(
            "/path/to/image.jpg", ["animals", "vehicles"]
        )

        self.assertEqual(
            result.category, "animals"
        )  # Should find "animals" in response
    ...
    def test_get_extraction_statistics_success(self):
        ...
        result = self.service.get_extraction_statistics(
    ...
    def test_update_average_confidence(self):
        ...
        self.assertAlmostEqual(
            self.service.stats["average_confidence"], expected_avg, places=2
        )
    ...
    def test_statistics_tracking(self):
        ...
        self.assertEqual(
            self.service.stats["extraction_methods_used"]["pattern_match"], 1
        )
