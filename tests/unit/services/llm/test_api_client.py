...
class TestRetryOnFailureDecorator(unittest.TestCase):
    ...
    def test_retry_on_exception_and_then_success(self):
        mock_func = Mock(side_effect=[requests.exceptions.RequestException("Error"), "Success"])
        mock_func.__name__ = "mock_func"
        decorated_func = retry_on_failure(max_retries=1, backoff_factor=0.1)(mock_func)
...
class TestLLMApiClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = LLMApiClient(self.api_url, self.config_service, log_level=logging.DEBUG)
    ...
    @patch('ai_content_classifier.services.llm.api_client.LLMApiClient._check_connection_internal', side_effect=requests.exceptions.RequestException("Generic request error"))
    ...
    @patch('ai_content_classifier.services.llm.api_client.LLMApiClient._check_connection_internal', side_effect=Exception("Unexpected error"))
    ...
    @patch('ai_content_classifier.services.llm.api_client.LLMApiClient._list_models_internal', side_effect=Exception("Unexpected model list error"))
    def test_list_models_general_exception(self, mock_internal):
        models = self.client.list_models()
        self.assertEqual(models, [])
        self.client.logger.error.assert_called_once_with("Failed to retrieve models: Unexpected model list error", exc_info=True)
    ...
    def test_pull_model_success(self, mock_sleep, mock_post):
        ...
        self.client.on_model_status_changed.assert_any_call("downloading", "Downloading test_model: 10.0%")
        self.client.on_model_status_changed.assert_any_call("downloading", "Downloading test_model: 50.0%")
        self.client.on_model_status_changed.assert_called_with("success", "Model test_model downloaded successfully")
    ...
    def test_pull_model_failure(self, mock_sleep, mock_post):
        ...
        self.client.on_model_status_changed.assert_called_once_with("error", "Failed to download model. HTTP 400: Bad Request")
    ...
    def test_pull_model_status_only_logging(self, mock_time, mock_sleep, mock_post):
        mock_time.side_effect = [0, 6, 12, 18, 20] # Simulate time passing to trigger log_interval and final time calculation
        ...
        result = self.client.pull_model("test_model")
        self.assertTrue(result)
        self.client.logger.info.assert_any_call("Model download status: pulling manifest")
        self.client.logger.info.assert_any_call("Model download status: verifying checksum")
        self.client.on_model_status_changed.assert_any_call("downloading", "pulling manifest")
        self.client.on_model_status_changed.assert_any_call("downloading", "verifying checksum")
        self.client.on_model_status_changed.assert_called_with("success", "Model test_model downloaded successfully")
    ...
    def test_pull_model_general_exception(self, mock_sleep, mock_post):
        result = self.client.pull_model("test_model")
        self.assertFalse(result)
        self.client.logger.error.assert_called_once_with("Failed to download model: Pull model unexpected error", exc_info=True)
        self.client.on_model_status_changed.assert_called_once_with("error", "Failed to download model: Pull model unexpected error")
    ...
    def test_generate_success(self, mock_sleep, mock_post):
        ...
        expected_json = {"model": "test_model", "prompt": "test prompt", "stream": False}
    ...
    def test_generate_with_response_format_schema(self, mock_sleep, mock_post):
        ...
        schema = {
            "type": "object",
            "properties": {"category": {"type": "string", "enum": ["Work", "Personal"]}},
            "required": ["category"],
        }
    ...
    @patch('requests.post', side_effect=requests.exceptions.RequestException("Generation error"))
    @patch('time.sleep', return_value=None)
    ...
    @patch('builtins.open', new_callable=unittest.mock.mock_open, read_data=b'image_data')
    @patch('requests.post')
    @patch('time.sleep', return_value=None)
    def test_generate_with_image(self, mock_sleep, mock_post, mock_open, mock_exists):
        ...
        response = self.client.generate("test_model", "analyze image", images=[image_path])
    ...
    def test_generate_with_non_existent_image(self, mock_sleep, mock_post, mock_exists):
        response = self.client.generate("test_model", "analyze image", images=["/tmp/non_existent.jpg"])
        
        # Test du comportement essentiel
        self.assertEqual(response, "")
        mock_post.assert_not_called()
        self.client.logger.warning.assert_called_once_with("Image file not found: /tmp/non_existent.jpg")

    @patch('os.path.exists', return_value=True)
    @patch('builtins.open', side_effect=IOError("File read error"))
    @patch('requests.post')
    @patch('time.sleep', return_value=None)
    def test_generate_image_encoding_exception(self, mock_sleep, mock_post, mock_open, mock_exists):
        image_path = "/tmp/test_image.jpg"
        response = self.client.generate("test_model", "analyze image", images=[image_path])
        self.assertEqual(response, "")
        self.client.logger.error.assert_called_once_with(f"Failed to encode image {image_path}: File read error")
        mock_post.assert_not_called()

    @patch('ai_content_classifier.services.llm.api_client.LLMApiClient._generate_internal', side_effect=Exception("Unexpected generation error"))
    def test_generate_general_exception(self, mock_internal):
        response = self.client.generate("test_model", "test prompt")
        self.assertEqual(response, "")
        self.client.logger.error.assert_called_once_with("Generation error: Unexpected generation error", exc_info=True)
