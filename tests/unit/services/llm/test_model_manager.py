...
from ai_content_classifier.services.llm.model_manager import (
    ModelStatus, ModelInfo, ModelCache, ModelManager
)
...
class TestModelCache(unittest.TestCase):
    ...
    def test_cache_cleanup(self):
        ...
        models = [
            ModelInfo(name=f"model{i}", status=ModelStatus.AVAILABLE)
            for i in range(5)
        ]
...
class TestModelManager(unittest.TestCase):
    ...
    def test_list_models_from_cache(self):
        ...
        # Pre-populate cache
        cached_models = [
            ModelInfo(name="cached_model", status=ModelStatus.AVAILABLE)
        ]
    ...
    def test_get_model_info_force_refresh(self):
        ...
        self.manager.get_model_info(
            self.test_api_url, "test_model", force_refresh=True
        )
    ...
    def test_pull_model_success(self):
        ...
        self.assertEqual(
            self.manager._download_status["test_model"]["status"], 
            "completed"
        )
    ...
    def test_pull_model_progress_callback(self):
        ...
        def simulate_progress(model_name):
            ...
            self.manager._handle_download_progress(model_name, "downloading", "50% complete")
            return True
    ...
    def test_handle_download_progress_with_percentage(self):
        ...
        self.manager._download_status["test_model"] = {"status": "starting", "progress": 0.0}
        self.manager._handle_download_progress("test_model", "downloading", "Downloaded 75%")
    ...
    def test_handle_download_progress_without_percentage(self):
        ...
        self.manager._download_status["test_model"] = {"status": "starting", "progress": 0.0}
        self.manager._handle_download_progress("test_model", "downloading", "Downloading...")
    ...
    def test_concurrent_operations(self):
        ...
        with patch.object(self.mock_api_client, 'pull_model', side_effect=mock_pull_model):
            ...
            for model_name in model_names:
                thread = threading.Thread(target=pull_model_concurrent, args=(model_name,))
            ...
            for model_name in model_names:
                ...
                self.assertEqual(self.manager._download_status[model_name]["status"], "completed")
    ...
    def test_cache_performance(self):
        ...
        self.assertEqual(
            stats['total_models'], self.manager.cache.max_cache_size + 10
        )
